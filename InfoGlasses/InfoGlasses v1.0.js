...
Item.defineArmor(499, "gg", 0, "Info Glasses","armor/gg.png", 1, 50, ArmorType.helmet);
Item.addShapedRecipe(499, 1, 0, ["b b","cac","b b"],["a", 264, 0, "b", 265, 0, "c", 102, 0]);
...
function ggCheck(){
	if(Player.getArmorSlot()==499){
		if(ggGUI==null){
			startggGUI();
			ggGUI!=null;
		}
	}
	else if(Player.getArmorSlot()!=499){
		if(ggGUI!=null){
			closeggGUI();
		}
	}
...
function startggGUI(){
	ctx.runOnUiThread(new java.lang.Runnable(){
		run: function(){
			try{
				ggGUI = new android.widget.PopupWindow();
				ggGUI.setWidth(375);
				ggGUI.setHeight(110);
				...
				cIdTv = new android.widget.TextView(ctx);
				cIdTv.setTextSize(20);
				cIdTv.setTextColor(android.graphics.Color.parseColor("#FFFFFF"));
				cIdTv.setTypeface(font);
				
				timeTv = new android.widget.TextView(ctx);
				timeTv.setTextSize(20);
				timeTv.setTextColor(android.graphics.Color.parseColor("#FFFFFF"));
				timeTv.setTypeface(font);
				
				xyzTv = new android.widget.TextView(ctx);
				xyzTv.setTextSize(20);
				xyzTv.setTextColor(android.graphics.Color.parseColor("#FFFFFF"));
				xyzTv.setTypeface(font);
				
				pbxyzTv = new android.widget.TextView(ctx);
				pbxyzTv.setTextSize(20);
				pbxyzTv.setTextColor(android.graphics.Color.parseColor("#FFFFFF"));
				pbxyzTv.setTypeface(font);
				
				pbIdTv = new android.widget.TextView(ctx);
				pbIdTv.setTextSize(20);
				pbIdTv.setTextColor(android.graphics.Color.parseColor("#FFFFFF"));
				pbIdTv.setTypeface(font);
				
				window.addView(cIdTv);
				window.addView(timeTv);
				window.addView(xyzTv);
				window.addView(pbxyzTv);
				window.addView(pbIdTv);
				
				ggGUI.setContentView(window);
				ggGUI.setBackgroundDrawable(new android.graphics.drawable.ColorDrawable(android.graphics.Color.TRANSPARENT));
				ggGUI.showAtLocation(ctx.getWindow().getDecorView(),android.view.Gravity.LEFT,25,-150);
			}
			catch(e){
				print(e);
			}
...
function updateggGUI(){
	var ciid = Player.getCarriedItem();
	var cid = Player.getCarriedItemData();
	var cic = Player.getCarriedItemCount();
	var time = Level.getTime();
	var x = Math.round(Player.getX());
	var y = Math.round(Player.getY());
	var z = Math.round(Player.getZ());
	var pbx = Math.round(Player.getPointedBlockX());
	var pby = Math.round(Player.getPointedBlockY());
	var pbz = Math.round(Player.getPointedBlockZ());
	var pbId = Math.round(Player.getPointedBlockId());
	var pbd = Math.round(Player.getPointedBlockData());
	ctx.runOnUiThread(new java.lang.Runnable(){
		run: function(){
			try{
				if(cIdTv!=null){
					cIdTv.setText("Carried Item Id: "+ciid+", "+cid+", "+cic);
				}
				if(timeTv!=null){
					timeTv.setText("Time: "+time);
				}
				if(xyzTv!=null){
					xyzTv.setText("X: "+x+", Y: "+y+", Z: "+z);
				}
				if(pbxyzTv!=null){
					pbxyzTv.setText("Pointed Block X: "+pbx+", Y: "+pby+", Z: "+pbz);
				}
				if(pbIdTv!=null){
					pbIdTv.setText("Pointed Block Id: "+pbId+", Data: "+pbd);
				}
			}
			catch(e){
				print(e);
